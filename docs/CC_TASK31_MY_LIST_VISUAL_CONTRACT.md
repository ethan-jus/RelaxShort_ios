...
- Following 下编辑按钮始终保留并可见；无收藏时禁用并降低透明度，不能隐藏后导致页签布局跳动。
...
- 按 `rank_position` 升序后只取前 6 个，固定三列两行；第 7 项及以后不显示。
- 三列自适应网格，水平边距 16pt，列间距 10pt，行间距约 18pt。
- 单卡海报保持项目统一海报比例和 `DB.posterRadius`。
- 海报左上角必须显示排名角标：1 为橙/金色、2 为绿色、3 为蓝色、4–6 为深灰色；角标约 26×26pt，数字白色。
- 热度值可用 flame + metric 叠在封面右下角。
- 标题 14pt、最多两行；副标签显示 category/首个 tag，13pt、单行、灰色。
